...
- Change: cold-started FlClash to restore core/TUN, then added a persistent narrow `api.multica.ai: 54.186.192.251` host override in both FlClash preferences and generated config. The single-string form follows FlClash 0.8.93's `Map<String, String>` schema; the address was present in the historical baseline, returned by Cloudflare/Google/AliDNS current checks, and passed five fixed-IP probes. No subscription, node, proxy group, DNS server, system proxy, Tailscale, or UU setting changed.
...
- Rollback: FlClash preference/config backups remain beside the originals with timestamps `20260711-210154` and `20260711-210348`. Do not use `kill -HUP FlClashCore` as a reload mechanism; use the app's apply/restart path or a full app cold start.
